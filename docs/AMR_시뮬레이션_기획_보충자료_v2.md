...
v2.1 추가(2026-08-26): v1 미션 모델 2종 확정 — 팔레트·대차 반송 + 토트 랙 피킹 투어(다중 토트 랙을 리프트로 싣고 피킹 존 순회, Locus식 운영을 iw.hub 하드웨어로 모델링). 투어는 사람–로봇 랑데부 대기가 지배라 DES 전담, SIL은 리프트 도킹 1장면만 추가
...
**(1) 문제 범위 정합**: 본 서비스의 대상은 기존 레이아웃 유지 하의 팔레트 반송 + 토트 랙 피킹 투어 도입(브라운필드) — 1.2절의 계약물류 3PL 타겟과 일치. Geek+의 시장 지위 원천인 GTP 그리드(바닥 QR + pod 운반)는 레이아웃을 재설계하는 그린필드 문제로, 스캔/도면 기반 경로망 생성이라는 본 파이프라인의 핵심 기술이 적용될 자리가 없다. → "업계 1위 제품군은 본 문제의 후보군에 애초에 없다."
