...
- **화물 드레싱** (만재): 바닥 파렛트 밴드(셀값 6)는 rect에서 래핑 파렛트 더미를
  자동 생성(교차 적재 2~3단 + 반투명 랩 합성, 빈 자리 3%), 랙은 바닥층 파렛트
  만재(2x2+토퍼)·데크층 2열 연속 적재+토퍼(랙 화물 약 6,500점 — 결정적
  의사난수 배치, USD 인스턴싱). 콜라이더는 그리드 rect 그대로(단일 소스)
